...
from numpy import (
	arcsin as asin, max, min, abs, sin, exp, pi, sqrt, inf,
	array, matrix
)

bivariate_normal_cdf = lambda h, k, r: bvnu(-h, -k, r)

phid = lambda phid z: erfc(-z/sqrt(2))/2
...
def bvnu(h, k, r):
	...
	elif h == -inf: return 1 if k == -inf else phid(-k)
	elif dk == -inf: return phid(-h)
	elif r == 0: return phid(-h)*phid(-k)
    ...
    elif abs(r) < 0.75 # Gauss Legendre points and weights, n = 12
